#include "MacWindowLevel.h"            // 包含macOS窗口层级控制头文件
...
void MacWindowLevel::setWindowAboveStatusBar(quintptr winId) {
  ...
  NSView *view =
      reinterpret_cast<NSView *>(winId); // 将Qt窗口ID转换为NSView指针
  ...
  qDebug() << "MacWindowLevel: 窗口层级已设置为屏保级别" << screenSaverLevel
           << "（高于所有系统UI）"; // 输出调试信息
}
...
void MacWindowLevel::setWindowPopupLevel(quintptr winId) {
  ...
  qDebug() << "MacWindowLevel: 窗口层级已设置为弹出菜单级别"
           << NSPopUpMenuWindowLevel; // 输出调试信息
}
...
void MacWindowLevel::disableSystemGestures(quintptr winId) {
  ...
  qDebug() << "MacWindowLevel: 已完全禁用系统手势和动画，窗口行为设置为"
           << behavior; // 输出调试信息
}
...
void MacWindowLevel::disableSystemHotCorners() {
  ...
  NSWindow *topBar = [[NSWindow alloc]
      initWithContentRect:NSMakeRect(0, screenRect.size.height - 2,
  ...
  [topBar
      setIgnoresMouseEvents:YES]; // 关键：让鼠标事件穿透，只阻挡系统手势检测
  [topBar setCollectionBehavior:
              NSWindowCollectionBehaviorCanJoinAllSpaces |
              NSWindowCollectionBehaviorStationary]; // 设置窗口行为
  [topBar orderFront:nil];                           // 显示窗口

  // 将窗口引用存储在用户默认值中（简单的全局存储方式）
  [[NSUserDefaults standardUserDefaults]
      setValue:@((long long)topBar)
        forKey:@"ScreenshotTopBarWindow"]; // 存储窗口引用

  qDebug()
      << "MacWindowLevel: 已创建顶部阻挡条（穿透模式），禁用状态栏下拉"; // 输出调试信息
}
...
void MacWindowLevel::restoreSystemHotCorners() {
  ...
  long long topBarPtr = [[[NSUserDefaults standardUserDefaults]
      valueForKey:@"ScreenshotTopBarWindow"]
      longLongValue];                         // 获取存储的窗口引用
  if (topBarPtr != 0) {                       // 如果窗口引用有效
    NSWindow *topBar = (NSWindow *)topBarPtr; // 转换为NSWindow指针
    [topBar close];                           // 关闭窗口
    [[NSUserDefaults standardUserDefaults]
        removeObjectForKey:@"ScreenshotTopBarWindow"]; // 移除存储的引用
    qDebug()
        << "MacWindowLevel: 已移除顶部阻挡条，恢复系统手势"; // 输出调试信息
  }
}