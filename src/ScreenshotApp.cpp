#include "jietu/ScreenshotApp.h"     // 包含截图应用程序头文件
#include "jietu/SystemTray.h"        // 包含系统托盘头文件
#include "jietu/ScreenshotOverlay.h" // 包含截图覆盖层头文件
#include "jietu/MacGlobalShortcut.h" // 包含全局快捷键头文件
...
// 截图应用程序构造函数
ScreenshotApp::ScreenshotApp(QObject *parent)
    : QObject(parent) // 调用基类构造函数
{
    ...
    // 连接信号
    connect(m_systemTray.get(), &SystemTray::screenshotRequested, // 连接截图请求信号
            this, &ScreenshotApp::startScreenshot);               // 到开始截图槽函数
    connect(m_systemTray.get(), &SystemTray::exitRequested,       // 连接退出请求信号
            this, &ScreenshotApp::exitApplication);               // 到退出应用程序槽函数

    // 创建全局快捷键 Cmd+Shift+A
    m_globalShortcut = new MacGlobalShortcut(this);

    // 连接快捷键到截图功能
    connect(m_globalShortcut, &MacGlobalShortcut::activated,
            this, &ScreenshotApp::startScreenshot);
...
void ScreenshotApp::startScreenshot()
{
    ...
    // 创建截图覆盖窗口
    m_screenshotOverlay = std::make_unique<ScreenshotOverlay>(m_fullScreenCapture); // 创建覆盖窗口对象

    // 连接信号
    connect(m_screenshotOverlay.get(), &ScreenshotOverlay::screenshotFinished,  // 连接截图完成信号
            this, &ScreenshotApp::onScreenshotFinished);                        // 到截图完成处理函数
    connect(m_screenshotOverlay.get(), &ScreenshotOverlay::screenshotCancelled, // 连接截图取消信号
            this, &ScreenshotApp::onScreenshotCancelled);                       // 到截图取消处理函数
    ...
    // 延迟提升窗口层级以确保覆盖状态栏
    QTimer::singleShot(50, m_screenshotOverlay.get(), [this]() // 延迟50毫秒执行
                       {
...
void ScreenshotApp::saveScreenshot(const QRect &region)
{
    ...
    // 调整区域坐标到实际像素
    QRect actualRegion(
        region.x() * devicePixelRatio,       // 调整X坐标
