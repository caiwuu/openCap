      ...
      m_exitAction(nullptr) // 初始化退出动作指针为空
{
    ...
    // 连接信号
    connect(this, &QSystemTrayIcon::activated,   // 连接托盘图标激活信号
            this, &SystemTray::onTrayActivated); // 到托盘激活处理函数
...
void SystemTray::createMenu()
{
    ...
    connect(m_screenshotAction, &QAction::triggered, // 连接动作触发信号
            this, &SystemTray::onScreenshotAction);  // 到截图动作处理函数

    // 创建退出动作
    m_exitAction = new QAction("退出", this);  // 创建退出动作，显示文字为"退出"
    connect(m_exitAction, &QAction::triggered, // 连接动作触发信号
            this, &SystemTray::onExitAction);  // 到退出动作处理函数
